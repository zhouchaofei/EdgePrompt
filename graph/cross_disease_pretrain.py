"""
跨疾病预训练脚本
实现GraphMAE在ABIDE和MDD数据集上的跨疾病预训练
"""
import os
import torch
import numpy as np
...
from graphmae_pretrain import GraphMAE, GraphMAETrainer
...
def pretrain_on_source(source_dataset, target_dataset, args):
    """
    在源数据集上预训练，用于目标数据集
    ...
    print(f"\n模型参数:")
    print(f"  编码器层数: {args.num_layer}")
    print(f"  隐藏维度: {args.hidden_dim}")
    print(f"  掩码率: {args.mask_rate}")
    print(f"  解码器隐藏维度: {args.decoder_hidden_dim}")
    ...
    save_path = os.path.join(
        args.save_dir,
        f'graphmae_{source_dataset}_for_{target_dataset}_{args.graph_method}.pth'
    )
...
def main():
    ...
    parser.add_argument('--mask_rate', type=float, default=0.75, help='节点掩码率')
    parser.add_argument('--decoder_hidden_dim', type=int, default=64, help='解码器隐藏维度')
