...
from graphmae_pretrain import GraphMAE
from abide_data import load_abide_data
from mdd_data import load_mdd_data
from load_data import GraphDownstream


def evaluate_pretrained_model(pretrained_path, target_dataset, args):
    """
    评估预训练模型在目标数据集上的性能
    """
    print(f"\n评估预训练模型: {pretrained_path}")
    ...
    # 添加分类头
    classifier = nn.Linear(args.hidden_dim, output_dim).to(device)
    ...
    for epoch in range(100):
        ...
        for batch in train_loader:
            ...
            # 获取图表示
            with torch.no_grad():
                graph_rep = model(batch, return_rep=True)
...
def main():
    ...
    # 评估ABIDE->MDD
    pretrained_path = f'./pretrained_models/graphmae/graphmae_ABIDE_for_MDD_{args.graph_method}.pth'
    acc = evaluate_pretrained_model(pretrained_path, 'MDD', args)
    print(f"\nABIDE->MDD: {acc:.4f}")

    # 评估MDD->ABIDE
    pretrained_path = f'./pretrained_models/graphmae/graphmae_MDD_for_ABIDE_{args.graph_method}.pth'
    acc = evaluate_pretrained_model(pretrained_path, 'ABIDE', args)
    print(f"\nMDD->ABIDE: {acc:.4f}")
